...
from vint_train.models.vint_xy.encoder import GoalEncoder
...
class ViNT(BaseModel):
    def __init__(
        ...
        if obs_encoder.split("-")[0] == "efficientnet":
            ...
            self.goal_encoder = GoalEncoder(
                input_dim=2, latent_dim=3000, mlp_hidden_dim=512, output_dim=256
            )
            self.num_goal_features = self.goal_encoder.out_features
        else:
    ...
    def forward(
        self, obs_img: torch.tensor, goal_xy: torch.tensor,
    ) -> Tuple[torch.Tensor, torch.Tensor]:

        # get the fused observation and goal encoding
        goal_encoding = self.goal_encoder.extract_features(goal_xy)
        # if self.late_fusion:
        #     goal_encoding = self.goal_encoder.extract_features(goal_xy)
        # else:
